...
import { WeeklyDietaryActivityProps } from '@/interface'

const WeeklyDietaryActivity = ({
  weeklyProgress,
  currentPage,
  totalPages,
  currentWeeks,
  ...
  handleNextPage,
  itemsPerPage,
}: WeeklyDietaryActivityProps) => {
  return (
    <Card className="shadow-md">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            <UtensilsCrossed className="h-5 w-5 text-primary" /> Weekly Dietary Activity
          </CardTitle>
      ...
      <CardContent className="space-y-6">
        {currentWeeks.map(week => (
          ...
          >
            <div className="flex justify-between text-sm">
              ...
              <span className="text-muted-foreground">{week.completed} days</span>
            </div>
        ...
        <div className="flex justify-between items-center pt-4 border-t">
          ...
          <div className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold">
            {(totalCompleted / (weeklyProgress.length || 1)).toFixed(1)} days
          </div>
        </div>
      </CardContent>
    </Card>
...
export default WeeklyDietaryActivity
